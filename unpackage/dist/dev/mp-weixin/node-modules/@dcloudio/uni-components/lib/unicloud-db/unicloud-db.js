...
const _sfc_main = {
  name: "UniClouddb",
  setup(props) {
    const dataListRef = props.ssrKey ? props.getone ? common_vendor.shallowSsrRef(void 0, props.ssrKey) : common_vendor.ssrRef([], props.ssrKey) : props.getone ? common_vendor.shallowSsrRef(void 0, "4tyskWttD5F2PYGzmJP0Fw==") : common_vendor.ssrRef([], "mtBnYa1MqjE8khfC557Jkg==");
  ...
  methods: {
    ...
    add(value, {
      ...
      let db = common_vendor.Ys.database(this.spaceInfo);
      if (action) {
        db = db.action(action);
      }
    ...
    _execRemove(id, action, success, fail, complete, needConfirm, needLoading, loadingTitle) {
      ...
      const db = common_vendor.Ys.database(this.spaceInfo);
